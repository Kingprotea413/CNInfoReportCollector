...
- `/.github/workflows/build-exe.yml`
  - 推送到 `main`、提交 Pull Request 或手动触发时，自动构建 Windows 和 macOS 安装包
  - Windows 会产出 `CNInfoReportCollector.exe` 和 zip
  - macOS 会产出 `CNInfoReportCollector.app` 的 zip 包
- `/.github/workflows/release.yml`
  - 推送形如 `v0.1.0` 的 tag 时，自动创建 GitHub Release
  - Release 会附带 Windows 的 `exe`、Windows zip 和 macOS zip
...
- macOS 构建目前是未签名、未 notarize 的 `.app` 压缩包
- 在其他 Mac 上首次打开时，系统可能会要求你右键选择“打开”或在系统设置里手动放行
